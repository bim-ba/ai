...
- **`uv` is the tooling prerequisite for the Claude Code side and all repo Python.** Both core hooks (SessionStart behaviour-protocol, Stop drift-log reminder) and `/setup` run Python through `uv run`; do not introduce `jq`, `yq`, or `npx`.
...
- **Hooks and custom agents follow** `plugins/core/templates/hooks-authoring-standard.md` and `agents-authoring-standard.md` (module-per-hook, wired through `hooks.json` for a plugin-shipped hook and `settings.json` for a project one; self-contained agent prompts with baked-in dispatch guards). Consulted on-demand, not scaffolded by `/setup`.
